...
let rec complementary_helix (x: helix) : helix =
  begin match x with
  | [] -> []
  | hd :: tl -> if hd = A then T :: complementary_helix tl
                else if hd = T then A :: complementary_helix tl
                else if hd = G then C :: complementary_helix tl
                else G :: complementary_helix tl
  end
...
let rec hamming_distance (x1: helix) (x2: helix) : int =
  ...
  | [], [] -> 0
  | [], _ -> failwith "two lists not equal number"
  | _, [] -> failwith "two lists not equal number"
...
let rec decreasing_similarity (apes: helix list) : bool =
  ...
  | [] -> true
  | [hd] -> true
  | hd1 :: hd2 :: tl -> hamming_distance human hd1 < hamming_distance human hd2 
  && decreasing_similarity (hd2 :: tl)
  end 
...
(* Helper function: put helices after Met in a list until END *)
let rec decode_after_met (x: helix) : acid list =
  begin match x with
  | [] -> []
  | [_] -> []
  | [_; _] -> [] 
  | hd1 :: hd2 :: hd3 :: tl -> if acid_of_triplet hd1 hd2 hd3 = END then [] 
                               else acid_of_triplet hd1 hd2 hd3 :: 
                               decode_after_met tl                           
  end
...
let rec acids_of_helix (x: helix) : acid list =
  ...
  | [_] -> []
  | [_; _] -> []
  | hd1 :: hd2 :: hd3 :: tl -> if acid_of_triplet hd1 hd2 hd3 = Met 
                              then Met :: decode_after_met tl
                              else acids_of_helix (hd2 :: hd3 :: tl)
  end
...
let rec add_ancestor_labels (t: tree) : labeled_tree =
  ...
  | Node (l, r) -> 
    ...
    | Leaf x, Node (l, r) -> 
      let var1 = add_ancestor_labels (Node (l, r)) in
      LNode (LLeaf x, guess_parent_helix x (helix_of_tree var1), var1)
    | Node (l, r), Leaf y -> 
      let var2 = add_ancestor_labels (Node (l, r)) in
      LNode (var2, guess_parent_helix (helix_of_tree var2) y, LLeaf y)
    | Node (l1, r1), Node (l2, r2) -> 
      let var3 = add_ancestor_labels (Node (l1, r1)) in
      let var4 = add_ancestor_labels (Node (l2, r2)) in
      LNode (var3, guess_parent_helix (helix_of_tree var3) (helix_of_tree var4),
        var4)
    end
  end
...
let rec parent_child_hamming (t: labeled_tree) : int =
  ...
  | LNode (lt, h, rt) -> 
    ...
    | LLeaf x, LLeaf y -> hamming_distance h x + hamming_distance h y
    | LLeaf x, LNode (l, hh, r) -> 
      hamming_distance h x +
      parent_child_hamming (LNode (l, hh, r)) + 
      hamming_distance h hh
    | LNode (l, hh, r), LLeaf y -> 
      hamming_distance h y +
      parent_child_hamming (LNode (l, hh, r)) + 
      hamming_distance h hh
    | LNode (l1, hh1, r1), LNode (l2, hh2, r2) -> 
      parent_child_hamming (LNode (l1, hh1, r1)) +
      parent_child_hamming (LNode (l2, hh2, r2)) + 
      hamming_distance h hh1 + 
      hamming_distance h hh2
    end
  end
(* -------------------------------------------------------------------------- *)
...
let rec simplest_tree (ts: labeled_tree list) : labeled_tree * int =
  ...
  | hd1 :: hd2 :: tl -> if parent_child_hamming hd1 < parent_child_hamming hd2 
                        then simplest_tree (hd1 :: tl)
                        else simplest_tree (hd2 :: tl)
  end
(* -------------------------------------------------------------------------- *)
...
let find_simplest_tree (x1: helix) (x2: helix) (x3: helix) (x4: helix) : tree =
  let ts1 = all_trees x1 x2 x3 x4 in
  let ts2 = add_ancestor_labels_list ts1 in
  let (t, _) = simplest_tree ts2 in
  unlabel_tree t
